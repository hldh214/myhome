...
def subscribe(client: mqtt_client.Client):
    def on_message(_client: mqtt_client.Client, userdata, msg):
        logger.info(f'Received `{msg.payload.decode()}` from `{triggered_topic}` topic, start leave home countdown')
        threading.Thread(target=presence_detection, args=(client,)).start()

    client.subscribe(triggered_topic)
    client.on_message = on_message


def presence_detection(client: mqtt_client.Client):
    alive = 0
    dead = 0
    ...
    client.publish(callback_topic, '{"status": "absent"}')
